...
class FlashMessagesDrawerAutoCloseTest < ApplicationSystemTestCase
  ...
  test "user sees flash message after updating chat in drawer. The flash message auto-dismisses after delay." do
    @drawer.open
    @drawer.assert_drawer_visible

    new_name = "Updated Chat Name"
    assert_selector "turbo-frame##{dom_id(@chat)}", visible: true, wait: 5
    ...
    sleep 2 # The test flash message has a default timeout of 1 second
