...
class ChatEditWithShiftTabTest < ApplicationSystemTestCase
  ...
  test "User edits chat name and submits with Shift+Tab key. Closes, reopens, and focuses on linkToChat" do
    @drawer.open
    ...
    focus_element_with_tab("##{second_chat_id} [data-chats--chat-list-target='editChatLink']")
    find("##{second_chat_id} [data-chats--chat-list-target='editChatLink']").send_keys(:enter)
    ...
    assert_equal "linkToChat", parsed_focus["target"]

    @drawer.close_by_escape
    @drawer.open
    @drawer.assert_drawer_visible

    assert has_selector?("##{second_chat_id} [data-chats--chat-list-target='linkToChat']:focus", wait: 5)
  end
end
